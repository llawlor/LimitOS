...
var outgoing_audio_server = new WebSocket.Server({port: 8082, perMessageDeflate: false});
...
incoming_audio_server.on('connection', function(socket, upgradeReq) {
  ...
  // get the auth token
  var auth_token = upgradeReq.url.split('/audio_from_devices/')[1];
  // add the auth token to the connection
  socket.auth_token = auth_token;
...
outgoing_audio_server.on('connection', function(socket, upgradeReq) {
  ...
  // get the auth token
  var auth_token = upgradeReq.url.split('/audio_to_clients/')[1];
  // add the auth token to the connection
  socket.auth_token = auth_token;
