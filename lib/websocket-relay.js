...
socketServer.on('connection', function(socket, upgradeReq) {
  ...
  if (socketServer.device_counts[auth_token] === undefined) {
    // add the device count, where the device is identified by the auth_token
    socketServer.device_counts[auth_token] = 1;
  // else there's already a count
	...
	console.log(
		...
		(upgradeReq || socket.upgradeReq).socket.remoteAddress,
		(upgradeReq || socket.upgradeReq).headers['user-agent'],
		'(auth_token: ' + auth_token + ', count: ' + socketServer.device_counts[auth_token] + ')'
	);
  ...
  // when the browser is disconnected
    ...
    socketServer.device_counts[auth_token] -= 1;
    // output the result
		console.log('Disconnected WebSocket (auth_token: ' + auth_token + ', count: ' + socketServer.device_counts[auth_token] + ')');
	});
